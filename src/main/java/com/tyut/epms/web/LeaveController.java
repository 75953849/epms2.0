...
public class LeaveController {
	...
	@RequestMapping("/toApproval1")
	public String toApproval1(@RequestParam(defaultValue="1") int page,@RequestParam(defaultValue="2") int rows,Leave leave,HttpServletRequest req) {
		System.out.println(leave);
		PageInfo<Leave> leaves = leaveService.findLeaveByPage(page, rows, leave);
		req.setAttribute("page", leaves);
		if(leave.getState()==null) {
